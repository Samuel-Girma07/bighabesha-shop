...
export async function initiateCheckout(
  ...
  const order = createOrder({
    ...
    amountETB,
    paymentRail: 'stars', // Default rail, updated when chosen
...
export async function renderPaymentRailSelection(ctx: Context, order: Order, productName: string): Promise<void> {
  ...
  const text = `🛒 *Checkout Confirmation*\n\n` +
    ...
    `⭐️ *Telegram Stars:* \`${starsDue} XTR\`\n` +
    `💎 *TON / USDT:* \`$${usdAmountWithMargin.toFixed(2)} USD\` (~${tonAmount} TON)\n\n` +
    `Please select your preferred payment method:`;

  const keyboard = new InlineKeyboard()
    .text(`⭐️ Telegram Stars (${starsDue} ⭐)`, `pay_stars_${order.id}`)
    .row()
...
export async function handleStarsPayment(ctx: Context, orderId: string): Promise<void> {
  ...
  // Send Telegram Stars invoice (currency XTR)
  try {
    ...
    await ctx.reply('⚠️ Failed to generate Telegram Stars invoice. Please try again or choose another payment rail.');
  }
}
...
export async function handleWalletPay(ctx: Context, orderId: string): Promise<void> {
  ...
  let text = `💎 *Wallet Pay (TON / USDT)*\n\n` +
    ...
    `Tap the button below to complete the transaction via Telegram Wallet:`;

  if (isMock) {
    text += `\n\n🛠 *Dev Mock Mode Active:*\nUse command \`/wp_simulate ${order.id}\` to simulate instantaneous payment confirmation!`;
  }
...
export async function handleManualRail(ctx: Context, rail: 'telebirr' | 'cbe' | 'abyssinia', orderId: string): Promise<void> {
  ...
  const text = `🏦 *Payment via ${railTitle}*\n\n` +
    ...
    `⚠️ *Important:* Take a screenshot of your successful transaction receipt or transfer confirmation SMS.\n\n` +
    `Once transferred, tap the **[I Paid — Upload Receipt]** button below to submit your proof.`;

  const keyboard = new InlineKeyboard()
    .text('📸 I Paid — Upload Receipt', `receipt_prompt_${order.id}`)
    .row()
...
export async function promptReceiptUpload(ctx: Context, orderId: string): Promise<void> {
  ...
  const text = `📸 *Submit Payment Receipt for Order \`${order.id}\`*\n\n` +
    `Please send a **photo / screenshot** of your payment confirmation in this chat.\n\n` +
    `_You can also include an optional note or transaction reference in the photo caption._`;
...
export async function performAdminApprove(ctx: Context, orderId: string): Promise<void> {
  ...
  try {
    ...
    // Edit current message to update status and remove action buttons
    ...
    if (autoDeliveredItem) {
      // Stock product (Gemini Pro) - Deliver payload immediately!
...
export async function promptAdminReject(ctx: Context, orderId: string): Promise<void> {
  ...
  const text = `❌ *Rejecting Order \`${orderId}\`*\n\n` +
    `Please type the reason for rejecting this receipt in chat (e.g. "Amount mismatch", "Invalid transaction reference", "Unreadable receipt"):\n\n` +
    `_This reason will be delivered directly to the buyer._`;
...
export async function notifyAdminsNewReceipt(ctx: Context, order: Order): Promise<void> {
  ...
  const caption = `🔔 *NEW RECEIPT PENDING APPROVAL*\n\n` +
    `• *Order ID:* \`${order.id}\`\n` +
    ...
    `Please review the receipt and approve or reject:`;
